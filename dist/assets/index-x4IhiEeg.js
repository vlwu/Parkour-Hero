 ...
 */function*V(r,e){if(r!==void 0){let t=0;for(const s of r)yield e(s,t++)}}function Z(r){return r===" "?"SPACE":r.startsWith("arrow")?r.replace("arrow","").toUpperCase():r.toUpperCase()}function Ae(r=0){const e=Math.floor(r/60),t=r%60,s=Math.floor(t),i=Math.floor((t-s)*1e3);return`${e.toString().padStart(2,"0")}:${s.toString().padStart(2,"0")}.${i.toString().padStart(3,"0")}`}class Zt extends w{static properties={fontRenderer:{type:Object},text:{type:String},scale:{type:Number},color:{type:String},outlineColor:{type:String},outlineWidth:{type:Number},align:{type:String}};constructor(){super(),this.text="",this.scale=1,this.color="white",this.outlineColor=null,this.outlineWidth=1,this.align="left"}updated(e){super.updated(e),!(!this.fontRenderer||!this.shadowRoot)&&this.renderCanvas()}renderCanvas(){const e=this.shadowRoot.querySelector("#container");if(!e)return;const t=this.fontRenderer.renderTextToCanvas(this.text,{scale:this.scale,color:this.color,outlineColor:this.outlineColor,outlineWidth:this.outlineWidth,align:this.align});t&&(t.style.imageRendering="pixelated",e.innerHTML="",e.appendChild(t))}render(){return m`<div id="container"></div>`}}customElements.define("bitmap-text",Zt);class Jt extends w{static styles=I`
  ...
  `;static properties={action:{type:String},currentKey:{type:String},isRemapping:{type:Boolean,state:!0},fontRenderer:{type:Object}};constructor(){super(),this.isRemapping=!1}connectedCallback(){super.connectedCallback(),window.addEventListener("keydown",this._handleGlobalKeydown)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("keydown",this._handleGlobalKeydown)}_handleGlobalKeydown=e=>{if(!this.isRemapping)return;e.preventDefault(),e.stopPropagation();const t=e.key.toLowerCase();this.dispatchEvent(new CustomEvent("keybind-changed",{detail:{action:this.action,newKey:t},bubbles:!0,composed:!0})),this.isRemapping=!1};_startRemap(e){e.stopPropagation(),this.isRemapping=!0,h.publish("playSound",{key:"button_click",volume:.8,channel:"UI"})}render(){const e=this.isRemapping?"Press key...":Z(this.currentKey);return m`
    ...
    `}}customElements.define("keybind-display",Jt);class Qt extends w{static styles=I`
    :host {
  ...
  `;static properties={keybinds:{type:Object},soundSettings:{type:Object},fontRenderer:{type:Object}};_dispatchClose(){h.publish("playSound",{key:"button_click",volume:.8,channel:"UI"}),this.dispatchEvent(new CustomEvent("close-modal",{bubbles:!0,composed:!0}))}_toggleSound(){h.publish("playSound",{key:"button_click",volume:.8,channel:"UI"}),h.publish("toggleSound")}_setVolume(e){const t=parseFloat(e.target.value);h.publish("setSoundVolume",{volume:t})}_testSound(){h.publish("playSound",{key:"jump",volume:.8,channel:"UI"})}render(){if(!this.keybinds||!this.soundSettings||!this.fontRenderer)return m``;const e=Object.keys(this.keybinds);return m`
    ...
    `}}customElements.define("settings-menu",Qt);class es extends w{static styles=I`
    :host {
      font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
    }
    .modal-overlay {
  ...
  `;static properties={stats:{type:Object},fontRenderer:{type:Object}};constructor(){super(),this.stats={collectedFruits:0,totalFruits:0,deathCount:0,levelTime:0}}_dispatch(e){this.dispatchEvent(new CustomEvent(e,{bubbles:!0,composed:!0}))}render(){return m`
    ...
    `}}customElements.define("pause-modal",es);class ts extends w{static styles=I`
    :host {
      font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
    }
    .modal-overlay {
  ...
  `;static properties={gameState:{type:Object},fontRenderer:{type:Object}};_dispatchClose(){h.publish("playSound",{key:"button_click",volume:.8,channel:"UI"}),this.dispatchEvent(new CustomEvent("close-modal",{bubbles:!0,composed:!0}))}_selectLevel(e,t){this.dispatchEvent(new CustomEvent("level-selected",{detail:{sectionIndex:e,levelIndex:t},bubbles:!0,composed:!0}))}_openStatsModal(){h.publish("playSound",{key:"button_click",volume:.8,channel:"UI"}),h.publish("ui_button_clicked",{buttonId:"stats"})}render(){return this.gameState?m`
    ...
    `:m``}}customElements.define("levels-menu",ts);class ss extends w{static styles=I`
    :host {
  ...
  `;static properties={characterId:{type:String},idleSprite:{type:Object},isLocked:{type:Boolean},isSelected:{type:Boolean},fontRenderer:{type:Object}};constructor(){super(),this.animationFrameId=null,this.animState={frame:0,timer:0,lastTime:0}}connectedCallback(){super.connectedCallback(),this.animationFrameId=requestAnimationFrame(this._animatePreview)}disconnectedCallback(){super.disconnectedCallback(),this.animationFrameId&&cancelAnimationFrame(this.animationFrameId)}_animatePreview=e=>{const t=this.shadowRoot.querySelector(".char-canvas");if(!t||!this.idleSprite){this.animationFrameId=requestAnimationFrame(this._animatePreview);return}this.animState.lastTime===0&&(this.animState.lastTime=e);const s=(e-this.animState.lastTime)/1e3;this.animState.lastTime=e,this.animState.timer+=s;const i=.08,n=11,a=this.idleSprite.width/n;if(this.animState.timer>=i){this.animState.timer=0,this.animState.frame=(this.animState.frame+1)%n;const o=t.getContext("2d");o.clearRect(0,0,t.width,t.height),o.drawImage(this.idleSprite,this.animState.frame*a,0,a,this.idleSprite.height,0,0,t.width,t.height)}this.animationFrameId=requestAnimationFrame(this._animatePreview)};_handleSelect(){this.isLocked||this.isSelected||this.dispatchEvent(new CustomEvent("character-selected",{detail:{characterId:this.characterId},bubbles:!0,composed:!0}))}render(){const e=J[this.characterId],t=`character-card ${this.isLocked?"locked":""} ${this.isSelected?"selected":""}`,s=this.isLocked?"Locked":this.isSelected?"Selected":"Select";return m`
    ...
    `}}customElements.define("character-card",ss);class is extends w{static styles=I`
    .modal-overlay {
      position: fixed; inset: 0;
      background-color: rgba(0, 0, 0, 0.7);
      display: flex; justify-content: center; align-items: center;
  ...
  `;static properties={gameState:{type:Object},assets:{type:Object},fontRenderer:{type:Object}};_dispatchClose(){h.publish("playSound",{key:"button_click",volume:.8,channel:"UI"}),this.dispatchEvent(new CustomEvent("close-modal",{bubbles:!0,composed:!0}))}render(){if(!this.gameState||!this.assets)return m`<div class="modal-overlay">Loading...</div>`;const e=Object.keys(J);return m`
    ...
    `}}customElements.define("character-menu",is);class ns extends w{static styles=I`
    :host {
      font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
    }
    .modal-overlay {
  ...
  `;static properties={keybinds:{type:Object},fontRenderer:{type:Object}};_dispatchClose(){h.publish("playSound",{key:"button_click",volume:.8,channel:"UI"}),this.dispatchEvent(new CustomEvent("close-modal",{bubbles:!0,composed:!0}))}render(){return this.keybinds?m`
      <div class="modal-overlay" @click=${this._dispatchClose}>
        <div class="modal-content" @click=${e=>e.stopPropagation()}>
          <button class="close-button" @click=${this._dispatchClose}></button>
    ...
    `:m``}}customElements.define("info-modal",ns);class as extends w{static styles=I`
    .modal-overlay {
      position: fixed; inset: 0;
      background-color: rgba(0, 0, 0, 0.7);
  ...
  `;static properties={stats:{type:Object},hasNextLevel:{type:Boolean},hasPreviousLevel:{type:Boolean},fontRenderer:{type:Object}};_dispatch(e){this.dispatchEvent(new CustomEvent(e))}render(){return this.stats?m`
    ...
    `:m``}}customElements.define("level-complete-modal",as);class os extends w{static styles=I`
    :host {
      font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
    }
    .modal-overlay {
  ...
  `;static properties={gameState:{type:Object},fontRenderer:{type:Object}};_dispatchClose(){h.publish("playSound",{key:"button_click",volume:.8,channel:"UI"}),this.dispatchEvent(new CustomEvent("close-modal",{bubbles:!0,composed:!0}))}_getStatDisplay(e,t=null){return e==null?"-":t?t(e):e.toString()}render(){if(!this.gameState||!this.gameState.levelStats)return m``;const{levelStats:e}=this.gameState;return m`
    ...
    `}}customElements.define("stats-modal",os);class rs extends w{static styles=I`
    .main-menu-overlay {
      position: fixed; inset: 0;
  ...
  `;static properties={activeModal:{type:String,state:!0},gameHasStarted:{type:Boolean,state:!0},keybinds:{type:Object,state:!0},soundSettings:{type:Object,state:!0},currentStats:{type:Object,state:!0},gameState:{type:Object,state:!0},assets:{type:Object,state:!0},fontRenderer:{type:Object},levelCompleteStats:{type:Object,state:!0}};constructor(){super(),this.activeModal="main-menu",this.gameHasStarted=!1,this.keybinds={moveLeft:"a",moveRight:"d",jump:"w",dash:" "},this.soundSettings={soundEnabled:!0,soundVolume:.5},this.currentStats={},this.gameState=null,this.assets=null,this.fontRenderer=null,this.levelCompleteStats=null}connectedCallback(){super.connectedCallback(),h.subscribe("requestStartGame",this._handleStartGame),h.subscribe("soundSettingsChanged",this._handleSoundUpdate),h.subscribe("keybindsUpdated",this._handleKeybindsUpdate),h.subscribe("ui_button_clicked",this._handleUIButtonClick),h.subscribe("statsUpdated",this._handleStatsUpdate),h.subscribe("action_escape_pressed",this._handleEscapePress),h.subscribe("levelLoaded",this._handleLevelLoad),h.subscribe("gameStateUpdated",e=>this.gameState=e),h.subscribe("assetsLoaded",e=>this.assets=e),h.subscribe("levelComplete",e=>this.levelCompleteStats=e)}disconnectedCallback(){super.disconnectedCallback(),h.unsubscribe("requestStartGame",this._handleStartGame),h.unsubscribe("soundSettingsChanged",this._handleSoundUpdate),h.unsubscribe("keybindsUpdated",this._handleKeybindsUpdate),h.unsubscribe("ui_button_clicked",this._handleUIButtonClick),h.unsubscribe("statsUpdated",this._handleStatsUpdate),h.unsubscribe("action_escape_pressed",this._handleEscapePress),h.unsubscribe("levelLoaded",this._handleLevelLoad),h.unsubscribe("gameStateUpdated",e=>this.gameState=e),h.unsubscribe("assetsLoaded",e=>this.assets=e),h.unsubscribe("levelComplete",e=>this.levelCompleteStats=e)}_handleLevelLoad=({gameState:e})=>{this.gameState=e,this.levelCompleteStats=null,this.gameHasStarted||(this.gameHasStarted=!0),this.activeModal=null};_handleStartGame=()=>{this.gameHasStarted=!0,this.activeModal=null,h.publish("allMenusClosed")};_handleSoundUpdate=e=>{this.soundSettings={...e}};_handleKeybindsUpdate=e=>{this.keybinds={...e}};_handleStatsUpdate=e=>{this.currentStats={...e}};_handleUIButtonClick=({buttonId:e})=>{e==="pause"?this.activeModal?this._closeModal():this.gameHasStarted&&(this.activeModal="pause",h.publish("menuOpened")):e==="stats"?(this.activeModal="stats",h.publish("menuOpened")):(this.activeModal=e,h.publish("menuOpened"))};_handleEscapePress=()=>{this.levelCompleteStats||(this.activeModal?this._closeModal():this.gameHasStarted&&(this.activeModal="pause",h.publish("menuOpened")))};_handleKeybindChange=e=>{const{action:t,newKey:s}=e.detail,i={...this.keybinds,[t]:s};h.publish("keybindsUpdated",i)};_closeModal=()=>{const e=this.activeModal!==null;this.activeModal=this.gameHasStarted?null:"main-menu",e&&this.gameHasStarted&&h.publish("allMenusClosed")};_openModalFromMenu(e){h.publish("playSound",{key:"button_click",volume:.8,channel:"UI"}),this.activeModal=e}_handleRestart(){this._closeModal(),h.publish("requestLevelRestart")}_handleOpenLevelsMenu(){this.activeModal="levels"}_handleLevelSelected(e){const{sectionIndex:t,levelIndex:s}=e.detail;h.publish("requestLevelLoad",{sectionIndex:t,levelIndex:s})}_handleCharacterSelected(e){const{characterId:t}=e.detail,s=this.gameState.setSelectedCharacter(t);s!==this.gameState&&(this.gameState=s,h.publish("gameStateUpdated",this.gameState)),h.publish("playSound",{key:"button_click",volume:.8,channel:"UI"}),h.publish("characterUpdated",t)}_handleLevelAction(e){this.levelCompleteStats=null,e==="restart"?h.publish("requestLevelRestart"):e==="next"?h.publish("requestNextLevel"):e==="previous"&&h.publish("requestPreviousLevel")}render(){return this.levelCompleteStats?m`
      ...
      `}renderMainMenuContent(){const e=[{text:"Start Game",action:()=>h.publish("requestStartGame")},{text:"Levels",action:()=>this._openModalFromMenu("levels")},{text:"Character",action:()=>this._openModalFromMenu("character")},{text:"Settings",action:()=>this._openModalFromMenu("settings")},{text:"Stats",action:()=>this._openModalFromMenu("stats")}];return m`
    ...
    `}renderActiveModal(){switch(this.activeModal){case"settings":return m`<settings-menu 
                    ...
                    ></info-modal>`;case"stats":return m`<stats-modal
                      ...
                      .fontRenderer=${this.fontRenderer}
                      @close-modal=${this._closeModal}
                    ></stats-modal>`;default:return m``}}}customElements.define("parkour-hero-ui",rs);const We=document.getElementById("ui-root");We?Ze(document.createElement("parkour-hero-ui"),We):console.error("UI Root element #ui-root not found. UI cannot be initialized.");const v=document.getElementById("gameCanvas"),y=v.getContext("2d");if(!v||!y)throw console.error("Canvas not found or context not available"),document.body.innerHTML="<h1>Error: Canvas not supported</h1>",new Error("Canvas not available");y.imageSmoothingEnabled=!1;const Je=1920,Qe=1080;v.width=Je;v.height=Qe;console.log(`Canvas initialized: ${Je}x${Qe}`);function et(){try{const r=1.7777777777777777,e=window.innerWidth/window.innerHeight;let t,s;e>r?(s=window.innerHeight,t=s*r):(t=window.innerWidth,s=t/r);const i=Math.floor(t),n=Math.floor(s);v.style.width=`${i}px`,v.style.height=`${n}px`,v.style.position="absolute",v.style.left=`${(window.innerWidth-i)/2}px`,v.style.top=`${(window.innerHeight-n)/2}px`,console.log(`Canvas resized to: ${i}x${n} (display size)`)}catch(r){console.error("Error resizing canvas:",r)}}window.addEventListener("resize",et);et();function ls(){y.fillStyle="#222",y.fillRect(0,0,v.width,v.height),y.fillStyle="white",y.font="24px sans-serif",y.textAlign="center",y.fillText("Loading Assets...",v.width/2,v.height/2);const r=300,e=20,t=(v.width-r)/2,s=v.height/2+30;y.strokeStyle="white",y.lineWidth=2,y.strokeRect(t,s,r,e),y.fillStyle="#4CAF50",y.fillRect(t,s,r*.1,e)}ls();let hs={moveLeft:"a",moveRight:"d",jump:"w",dash:" "},$,cs;Tt().then(r=>{console.log("Assets loaded successfully, preparing main menu...");try{const e=new Et(r.font_spritesheet);$=new wt(y,v,r,hs,e),h.publish("assetsLoaded",r);const t=document.querySelector("parkour-hero-ui");t&&(t.fontRenderer=e),cs=new At,h.subscribe("requestStartGame",()=>{$.start()}),window.unlockAllLevels=()=>{$&&$.gameState&&($.gameState.unlockAllLevels(),h.publish("gameStateUpdated",$.gameState))},console.log("Developer command available: Type `unlockAllLevels()` in the console to unlock all levels."),window.resetProgress=()=>{$&&$.gameState&&($.gameState.resetProgress(),$.loadLevel(0,0),console.log("Game reset to Level 1."),h.publish("gameStateUpdated",$.gameState))},console.log("Developer command available: Type `resetProgress()` in the console to reset all saved data."),console.log("Game is ready. Waiting for user to start from the main menu.")}catch(e){console.error("Failed to start game engine:",e),y.fillStyle="#222",y.fillRect(0,0,v.width,v.height),y.fillStyle="red",y.font="24px sans-serif",y.textAlign="center",y.fillText("Game Failed to Start",v.width/2,v.height/2-20),y.fillStyle="white",y.font="16px sans-serif",y.fillText("Check console for details",v.width/2,v.height/2+20)}}).catch(r=>{console.error("Asset loading failed:",r),y.fillStyle="#222",y.fillRect(0,0,v.width,v.height),y.fillStyle="red",y.font="24px sans-serif",y.textAlign="center",y.fillText("Failed to Load Assets",v.width/2,v.height/2-20),y.fillStyle="white",y.font="16px sans-serif",y.fillText("Check console for details",v.width/2,v.height/2+20)});window.addEventListener("error",r=>{console.error("Global error:",r.error)});window.addEventListener("unhandledrejection",r=>{console.error("Unhandled promise rejection:",r.reason)});console.log("Game initialization started");console.log("Canvas dimensions:",v.width,"x",v.height);console.log("Device pixel ratio:",window.devicePixelRatio);console.log("User agent:",navigator.userAgent);
