 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */function*K(r,e){if(r!==void 0){let t=0;for(const s of r)yield e(s,t++)}}function q(r){return r===" "?"SPACE":r.startsWith("arrow")?r.replace("arrow","").toUpperCase():r.toUpperCase()}function Te(r=0){const e=Math.floor(r/60),t=r%60,s=Math.floor(t),i=Math.floor((t-s)*1e3);return`${e.toString().padStart(2,"0")}:${s.toString().padStart(2,"0")}.${i.toString().padStart(3,"0")}`}class Kt extends w{static properties={fontRenderer:{type:Object},text:{type:String},scale:{type:Number},color:{type:String},outlineColor:{type:String},outlineWidth:{type:Number},align:{type:String}};constructor(){super(),this.text="",this.scale=1,this.color="white",this.outlineColor=null,this.outlineWidth=1,this.align="left"}updated(e){super.updated(e),!(!this.fontRenderer||!this.shadowRoot)&&this.renderCanvas()}renderCanvas(){const e=this.shadowRoot.querySelector("#container");if(!e)return;const t=this.fontRenderer.renderTextToCanvas(this.text,{scale:this.scale,color:this.color,outlineColor:this.outlineColor,outlineWidth:this.outlineWidth,align:this.align});t&&(t.style.imageRendering="pixelated",e.innerHTML="",e.appendChild(t))}render(){return m`<div id="container"></div>`}}customElements.define("bitmap-text",Kt);class Vt extends w{static styles=L`
    .keybind-display {
            ...
            ${K(A,(e,t)=>m`
              <div class="level-section-menu">
                ...
                <div class="level-grid">
                  ${K(e.levels,(s,i)=>{const n=this.gameState.isLevelUnlocked(t,i),a=this.gameState.isLevelCompleted(t,i),o=this.gameState.currentSection===t&&this.gameState.currentLevelIndex===i,l=`level-button ${a?"completed":""} ${o?"current":""} ${n?"":"locked"}`;return n?m`<button class=${l} @click=${()=>this._selectLevel(t,i)}>${i+1}</button>`:m`<button class=${l} disabled>
            ...
            ${K(A,(t,s)=>m`
              <div class="level-section-stats">
                ...
                <div class="stats-grid">
                  ...
                  ${K(t.levels,(i,n)=>{const a=`${s}-${n}`,o=e[a]||{fastestTime:null,lowestDeaths:null,totalAttempts:0};return m`
                        <div class="stat-row">
                            <div class="stat-cell level-name">Level ${n+1}</div>
                            <div class="stat-cell">${this._getStatDisplay(o.fastestTime,Te)}</div>
