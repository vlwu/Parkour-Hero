 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */function*ie(a,e){if(a!==void 0){let t=0;for(const s of a)yield e(s,t++)}}function G(a){return a===" "?"SPACE":a.startsWith("arrow")?a.replace("arrow","").toUpperCase():a.toUpperCase()}function Ne(a=0){const e=Math.floor(a/60),t=a%60,s=Math.floor(t),i=Math.floor((t-s)*1e3);return`${e.toString().padStart(2,"0")}:${s.toString().padStart(2,"0")}.${i.toString().padStart(3,"0")}`}class Ut extends w{static properties={fontRenderer:{type:Object},text:{type:String},scale:{type:Number},color:{type:String},outlineColor:{type:String},outlineWidth:{type:Number},align:{type:String}};constructor(){super(),this.text="",this.scale=1,this.color="white",this.outlineColor=null,this.outlineWidth=1,this.align="left"}updated(e){super.updated(e),!(!this.fontRenderer||!this.shadowRoot)&&this.renderCanvas()}renderCanvas(){const e=this.shadowRoot.querySelector("#container");if(!e)return;const t=this.fontRenderer.renderTextToCanvas(this.text,{scale:this.scale,color:this.color,outlineColor:this.outlineColor,outlineWidth:this.outlineWidth,align:this.align});t&&(t.style.imageRendering="pixelated",e.innerHTML="",e.appendChild(t))}render(){return f`<div id="container"></div>`}}customElements.define("bitmap-text",Ut);class Ot extends w{static styles=A`
    .keybind-display {
