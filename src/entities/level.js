...
export class Level {
  constructor(levelConfig, entityManager) {
    ...
    (levelConfig.objects || []).forEach(obj => {
      ...
      if (staticPlatformDefinitions[obj.type]) {
          this.staticPlatforms.push(new StaticPlatform(worldX, worldY, obj));
          return;
      }
  ...
  isSolidAt(worldX, worldY, ignoreOneWay = false) {
    ...
    for (const obj of potentialColliders) {
      ...
      if (obj.instance && obj.instance.solid && (obj.type === 'trap' || obj.type === 'static_platform')) {
