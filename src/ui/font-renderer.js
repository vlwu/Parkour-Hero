const FONT_MAP = {
    ...
    '(': { x: 40, y: 40 }, ')': { x: 48, y: 40 }, '+': { x: 56, y: 40 }, '-': { x: 64, y: 40 }, '/': { x: 72, y: 20 }, // Assuming '/' is next to Z
    ' ': { x: 0, y: 0, space: true } // Special case for space
};
const CHAR_WIDTH = 8;
...
export class FontRenderer {
    constructor(fontSprite) {
        ...
        // The cache for pre-rendered colored characters. This is the key to the performance fix.
        this.characterCache = new Map();
    }

    /**
     * Retrieves a pre-rendered, colored character from the cache.
     * If not in the cache, it creates it, stores it, and then returns it.
     */
    _getCachedCharacter(char, color) {
        ...
        // If not cached, create the colored character on a new canvas.
        ...
        // Perform the expensive coloring operation ONCE.
    ...
    _renderText(ctx, text, startX, startY, { scale = 1, color = null } = {}) {
        ...
        for (const char of upperText) {
            ...
            if (color) {
                // Get the pre-rendered colored character from the cache. This is very fast.
                imageToDraw = this._getCachedCharacter(char, color);
                sx = 0; // The cached image is a standalone character, so source is always (0,0)
                sy = 0;
            } else {
                // If no color, just use the original spritesheet.
            ...
            if (imageToDraw) {
                ctx.drawImage(
                    imageToDraw,
                    sx, sy, CHAR_WIDTH, CHAR_HEIGHT, // source rect
                    currentX, startY, CHAR_WIDTH * scale, CHAR_HEIGHT * scale // destination rect
                );
            }
