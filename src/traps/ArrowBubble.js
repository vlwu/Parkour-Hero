...
export class ArrowBubble extends Trap {
    constructor(x, y, config) {
        ...
        this.idleAnimation = { frameCount: 10, frameSpeed: 0.1, frameTimer: 0, currentFrame: 0 };
    ...
    update(dt, playerData, eventBus) {
        if (this.state === 'idle') {
            ...
            if (this.respawnTimer <= 0) this._resetToIdle();
        }
    }
    ...
    onCollision(player, eventBus) {
        ...
        if (isVertical) { ctrl.vLock = true; vel.vx = 0; }
        else { ctrl.hLock = true; vel.vy = 0; }

        switch (this.direction) {
            case 'up': vel.vy = -this.knockbackSpeed; break;
            case 'down': vel.vy = this.knockbackSpeed; break;
            case 'left': vel.vx = -this.knockbackSpeed; break;
            case 'right': vel.vx = this.knockbackSpeed; break;
        }
    }
